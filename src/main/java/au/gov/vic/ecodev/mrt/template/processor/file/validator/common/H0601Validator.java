...
public class H0601Validator implements Validator {

	public static final String SAMPLE_TYPE_TITLE = "Sample_Type";
	private static final String FIELD_NAME = "H0601";
	...
	public Optional<List<String>> validate(Map<String, List<String>> templateParamMap, Template dataBean) {
		...
		new TemplateFieldBasicValidator(strs, SAMPLE_TYPE_TITLE, FIELD_NAME)
			.validate(messages);
		boolean hasErrorMessage = new ErrorMessageChecker(messages).isContainsErrorMessages();
		if (!hasErrorMessage) {
			new ParamHelper(strs, Strings.TITLE_PREFIX + SAMPLE_TYPE_TITLE, templateParamMap)
				.addParamToMap();
		}
