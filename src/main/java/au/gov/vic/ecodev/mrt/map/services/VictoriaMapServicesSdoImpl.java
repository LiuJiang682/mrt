...
public class VictoriaMapServicesSdoImpl implements VictoriaMapServices {
	...
	private static final String MGA_54_NE_SQL = "SELECT COUNT(*) FROM FR_FRAMEWORK_AREA_POLYGON f WHERE MDSYS.SDO_CONTAINS(f.SHAPE, MDSYS.SDO_GEOMETRY(2001, 28354, MDSYS.SDO_POINT_TYPE(?, ?, NULL), NULL, NULL)) = 'TRUE' AND f.STATE = 'VIC'";
	private static final String MGA_55_NE_SQL = "SELECT COUNT(*) FROM FR_FRAMEWORK_AREA_POLYGON f WHERE MDSYS.SDO_CONTAINS(f.SHAPE, MDSYS.SDO_GEOMETRY(2001, 28355, MDSYS.SDO_POINT_TYPE(?, ?, NULL), NULL, NULL)) = 'TRUE' AND f.STATE = 'VIC'";
