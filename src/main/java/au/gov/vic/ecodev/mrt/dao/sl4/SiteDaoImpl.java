...
public class SiteDaoImpl implements SiteDao {
	...
	private static final String UPDATE_SQL = "UPDATE lOC_SITE SET GSV_SITE_ID=?, PARISH=?, PROSPECT=?, AMG_ZONE=?, EASTING=?, NORTHING=?, LATITUDE=?, LONGITUDE=?, LOCN_ACC=?, LOCN_DATUM_CD=?, ELEVATION_GL=?, ELEV_ACC=?, ELEV_DATUM_CD=?, NUM_DATA_RECORDS=?, ISSUE_COLUMN_INDEX = ? WHERE LOADER_ID = ? AND SITE_ID = ?";

	private static final String INSERT_SQL = "INSERT INTO lOC_SITE(LOADER_ID, SITE_ID, GSV_SITE_ID, PARISH, PROSPECT, AMG_ZONE, EASTING, NORTHING, LATITUDE, LONGITUDE, LOCN_ACC, LOCN_DATUM_CD, ELEVATION_GL, ELEV_ACC, ELEV_DATUM_CD, NUM_DATA_RECORDS, ISSUE_COLUMN_INDEX) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	private static final String COUNT_SQL = "SELECT COUNT(LOADER_ID) FROM lOC_SITE WHERE LOADER_ID = ? AND SITE_ID = ?";
	
	private static final String SELECT_SQL = "SELECT LOADER_ID, SITE_ID, GSV_SITE_ID, PARISH, PROSPECT, AMG_ZONE, EASTING, NORTHING, LATITUDE, LONGITUDE, LOCN_ACC, LOCN_DATUM_CD, ELEVATION_GL, ELEV_ACC, ELEV_DATUM_CD, NUM_DATA_RECORDS, ISSUE_COLUMN_INDEX FROM lOC_SITE WHERE LOADER_ID = ? AND SITE_ID = ?";
	...
	public boolean updateOrSave(Entity entity) {
		...
		if (Numeral.ZERO == count) {
			int rows = jdbcTemplate.update(INSERT_SQL, new Object[] {site.getLoaderId(), site.getSiteId(),
					...
					site.getElevDatumCd(), site.getNumberOfDataRecord(), site.getIssueColumnIndex()});
