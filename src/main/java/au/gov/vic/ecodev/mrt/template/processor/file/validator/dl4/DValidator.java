...
public class DValidator implements Validator {
	...
	public Optional<List<String>> validate(Map<String, List<String>> templateParamMap, Template dataBean) {
		...
		if (Numeral.INVALID_LINE_NUMBER != lineNumber) {
			if (null == strs) {
				String message = "Line " + lineNumber + " requires minimum 3 columns, only got 0";
				messages.add(message);
			} else if (strs.length < Numeral.THREE) {
				String message = "Line " + lineNumber + " requires minimum 3 columns, only got " + strs.length;
				messages.add(message);
			} else {
				...
				if (Numeral.INVALID_COLUMN_COUNT != columnHeaderCount) {
					new DepthFromNumberValidator(strs, lineNumber,
