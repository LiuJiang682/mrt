...
public class H0602ValidatorTest {

	private H0602Validator testInstance;
	private Template mockDataBean;

	@Test
	public void shouldReturnEmptyErrorMessage() {
		// Given
		String[] strs = { "H0602", "Sample_description", "DD"};
		givenTestInstance(strs);
		Map<String, List<String>> params = new HashMap<>();
		params.put(Strings.TITLE_PREFIX + H0600Validator.SAMPLE_CODE_TITLE, 
				Arrays.asList("DD"));
		params.put(Strings.TITLE_PREFIX + H0601Validator.SAMPLE_TYPE_TITLE, 
				Arrays.asList("DD"));
		// When
		...
		List<String> drillingCodes = params.get(Strings.TITLE_PREFIX +
				H0602Validator.DESCRIPTION_TITLE);
		...
		assertThat(keyCaptor.getValue(), is(equalTo("H0602")));
		MrtTemplateValue value = valueCaptor.getValue();
		assertThat(value, is(notNullValue()));
		List<String> values = value.getDatas();
		assertThat(values.isEmpty(), is(false));
		assertThat(values.get(0), is(equalTo("Sample_description")));
	...
	public void shouldReturnEmptyErrorMessageAnd2DrillingCodes() {
		// Given
		String[] strs = { "H0602", "Sample_description", "DD", "RC" };
		givenTestInstance(strs);
		Map<String, List<String>> params = new HashMap<>();
		params.put(Strings.TITLE_PREFIX + H0600Validator.SAMPLE_CODE_TITLE, 
		...
		assertThat(errorMessages.isPresent(), is(false));
		assertThat(params.isEmpty(), is(false));
		List<String> drillingCodes = params.get(Strings.TITLE_PREFIX + H0602Validator.DESCRIPTION_TITLE);
	...
	public void shouldReturnIncorrectLengthMessage() {
		...
		assertThat(messages.get(0), is(equalTo("ERROR: H0602 requires minimum 3 columns, only got 2")));
	...
	public void shouldReturnIncorrectLengthMessage0() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(2)));
		assertThat(messages.get(0), is(equalTo("ERROR: H0602 requires minimum 3 columns, only got 0")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
	...
	public void shouldReturnIncorrectStatementMessage() {
		...
		assertThat(messages.get(0), is(equalTo("ERROR: H0602 title must be Sample_description, but got dd")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}

	private void givenTestInstance(final String[] strs) {
		testInstance = new H0602Validator();
