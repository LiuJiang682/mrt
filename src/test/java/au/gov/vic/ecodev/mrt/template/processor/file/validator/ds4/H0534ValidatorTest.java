...
public class H0534ValidatorTest {

	private H0534Validator testInstance;
	...
	public void shouldReturnNoErrorMessage() {
		// Given
		String[] strs = { "H0534", "Downhole_Direction_Survey_instrument", "SS" };
		...
		assertThat(keyCaptor.getValue(), is(equalTo("H0534")));
		List<String> values = valueCaptor.getValue();
		assertThat(values.isEmpty(), is(false));
		assertThat(values.size(), is(equalTo(2)));
		assertThat(values.get(0), is(equalTo("Downhole_Direction_Survey_instrument")));
		assertThat(values.get(1), is(equalTo("SS")));
		assertThat(CollectionUtils.isEmpty(params.get("H0534")), is(false));
	}
	...
	public void shouldReturnNoErrorMessageWith2Strings() {
		// Given
		String[] strs = { "H0534", "Downhole_Direction_Survey_instrument" };
		givenTestInstance(strs);
		Map<String, List<String>> params = new HashMap<>();
		// When
		Optional<List<String>> errorMessages = testInstance.validate(params, mockDataBean);
	...
	public void shouldReturnIncorrectLengthMessage0() {
		...
		assertThat(messages.get(0), is(equalTo("H0534 requires minimum 2 columns, only got 0")));
	...
	public void shouldReturnIncorrectLengthMessage() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(1)));
		assertThat(messages.get(0), is(equalTo("H0534 requires minimum 2 columns, only got 1")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
	...
	public void shouldReturnIncorrectStatementMessage() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(1)));
		final String expectedMsg = "H0534 title must be Downhole_Direction_Survey_Instrument, but got dd";
	...
	private void givenTestInstance(final String[] strs) {
		testInstance = new H0534Validator();
