...
import au.gov.vic.ecodev.mrt.template.processor.file.validator.ds4.H0202Validator;
...
public class H0202ValidatorTest {
	...
	public void shouldReturnEmptyErrorMessage() {
		// Given
		String[] strs = { "H0202", "Data_format", "DS4" };
		...
		assertThat(values.get(1), is(equalTo("DS4")));
	}
	...
	public void shouldReturnIncorrectLengthMessage() {
		...
		assertThat(messages.get(0), is(equalTo("H0202 requires minimum 3 columns, only got 2")));
	...
	public void shouldReturnIncorrectLengthMessage0() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(1)));
		assertThat(messages.get(0), is(equalTo("H0202 requires minimum 3 columns, only got 0")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
	...
	public void shouldReturnIncorrectStateMessage() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(1)));
		assertThat(messages.get(0), is(equalTo("H0202 must be DS4, but got SA")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
	...
	public void shouldReturnIncorrectStatementMessage() {
		...
		List<String> messages = errorMessages.get();
		assertThat(messages.size(), is(equalTo(1)));
		assertThat(messages.get(0), is(equalTo("H0202 title must be Data_format, but got dd")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
	...
	public void shouldReturn2Message() {
		...
		assertThat(messages.get(0), is(equalTo("H0202 must be DS4, but got SA")));
		assertThat(messages.get(1), is(equalTo("H0202 title must be Data_format, but got dd")));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
	}
