...
@PrepareForTest({H0203H0532H0533DataIntegrityValidator.class, MessageHandler.class})
public class H0203H0532H0533DataIntegrityValidatorTest {

	private H0203H0532H0533DataIntegrityValidator testInstance;
	...
	public void shouldReturnEmptyMessageWhenStringArrayIsEof() throws Exception {
		...
		templateParamMap.put(Strings.KEY_H0532, Arrays.asList("Surface_Location_Survey_Instrument", "GPS"));
		templateParamMap.put(Strings.KEY_H0533, Arrays.asList("Surface_Location_Survey_Company"));
		// When
	...
	@Test
	public void shouldReturnMissingH0532MessageWhenStringArrayIsEofAndNoH0532Params() throws Exception {
		// Given
		givenTestInstance();
		templateParamMap = new HashMap<>();
		templateParamMap.put(Strings.NUMBER_OF_DATA_RECORDS_TITLE, Arrays.asList("3"));
		...
		assertThat(logSeverityCaptor.getValue(), is(equalTo(LogSeverity.ERROR)));
		String message = messageCaptor.getValue();
		assertThat(message.contains("No H0532 in the templateParamMap!"), is(true));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
		assertThat(dataBean, is(nullValue()));
	}
	
	@Test
	public void shouldReturnMissingH0533MessageWhenStringArrayIsEofAndNoH0533Params() throws Exception {
		// Given
		givenTestInstance();
		templateParamMap = new HashMap<>();
		templateParamMap.put(Strings.NUMBER_OF_DATA_RECORDS_TITLE, Arrays.asList("3"));
		...
		assertThat(logSeverityCaptor.getValue(), is(equalTo(LogSeverity.ERROR)));
		String message = messageCaptor.getValue();
		assertThat(message.contains("No H0533 in the templateParamMap!"), is(true));
		verify(mockDataBean, times(0)).put(Matchers.anyString(), Matchers.anyListOf(String.class));
		assertThat(dataBean, is(nullValue()));
	}
	...
	private void givenTestInstance() throws Exception {
		testInstance = new H0203H0532H0533DataIntegrityValidator();
